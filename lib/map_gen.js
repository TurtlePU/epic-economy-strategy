var support = {
	getProb: (height) => (depth) => Math.abs(depth) / height,
	resourceMap: (pack) => {
		var map = MapGen.diamondSquare(pack.a, pack.b, pack.c,
			pack.mod, pack.seed, pack.logSize, pack.height);
		var gen = new support.PseudoRandom(pack.prob_a, pack.prob_b, pack.prob_c,
			pack.prob_mod, pack.prob_seed);
		var getProb = support.getProb(pack.height);
	...
	distributedResourceMap: (pack) => {
		var map = support.resourceMap(pack);
	...
	PseudoRandom: function(a, b, c, mod, elem) {
		var prev = 0;
		this.getNext = () => {
			let t = (a * elem + b * prev + c) % mod;
			prev = elem;
			elem = t;
			return t;
		};
		this.getRanged = (from, to) => from + (to - from) * (this.getNext() / mod);
	}
};

var MapGen = {
	pack: (array, chunkWidth, chunkHeight) => {
		let sx = array.length, sy = array[0].length;
	...
	buildChunked: (data) => MapGen.pack(support.distributedResourceMap(data), data.chunkWidth, data.chunkHeight),
	chunkedDS: (pack) => {
		return MapGen.pack(
			support.normalize(
				MapGen.diamondSquare(
	...
	diamondSquare: function(a, b, c, mod, seed, log_size, height) {
		var gen = new support.PseudoRandom(a, b, c, mod, seed);
